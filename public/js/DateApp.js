if (String.prototype.getBaseConversionNumber === undefined) {
...
const calendarWrapperDOM = document.getElementsByClassName("calendar-wrapper");
...
const parentMetaMonthsDOM = document.getElementsByClassName('months')[0];
...
for (const element of monthLetter) {
    element.onclick = function (e) {
        ...
        if (thisElement.classList.contains("active-season-cr"))
            return;
...
for (const month of calendarObject) {
    ...
    for (const day of month) {
        ...
        if (!oneStarted)
            ...
            liClass += `${metaYear.year}-${monthCounter}-${day[0].convertDigits("en")} `;
        }
        ...
        ulCurrent.innerHTML += generateTemplateHTML('date', {
            class: liClass,
            fulldate: `${metaYear.year}-${monthCounter}-${day[0].convertDigits("en")}`,
...
activeMonthElement('calendar-base', 'calendar-left', `active-session-${todayFa.month}`);
...
function generateTemplateHTML(type, data) {
    ...
    if (type == "date")
        htmlTemplate = `<li class="${data.class}" fulldate="${data.fulldate}"><span id="jalali">${data.jalali}</span><small id="miladi">${data.miladi}</small><small id="ghamari">${data.ghamari}</small></li>`;
...
window.onkeyup = function (e) {
    ...
    var activeMonthDOM = document.getElementsByClassName("active-season-cr")[0];
